...
describe('release proof evidence index', () => {
	test('references top claim proof artifacts by digest without embedding artifacts', async () => {
		...
		expect(agentWorkflowMissing).toContain('prepared-plan rollback/retry evidence is enough')
		...
		expect(agentWorkflowDecision?.allowedWording).toContain('prepared-plan failure-recovery')
		expect(agentWorkflowNextOwnerAction).toContain('stops adding more prepared-plan')
