	...
	test('references top claim proof artifacts by digest without embedding artifacts', async () => {
		...
		for (const evidence of [
			...
			expect.objectContaining({
				...
				acceptedScope: expect.stringMatching(/8357e5de.*2d02e2c3/),
				command: expect.stringContaining('moveRange rewrites sheet autoFilter metadata'),
			...
			expect.objectContaining({
				...
				acceptedScope: expect.stringContaining('a2c2bd3f'),
				command: expect.stringContaining('resizeTable rejects protected table growth'),
				boundary: expect.stringContaining('Local table resize protection evidence only'),
			}),
			expect.objectContaining({
			...
			expect.objectContaining({
				...
				acceptedScope: expect.stringMatching(/824e57c5.*3e79423d.*aa94cbd0.*25d08758.*9cd32ae4/),
				command: expect.stringMatching(
					/parses workbook views, workbook properties, and external references.*clones connection and external-link inventories without aliasing nested metadata|clones connection and external-link inventories without aliasing nested metadata.*parses workbook views, workbook properties, and external references/,
				),
				boundary: expect.stringContaining(
					'Generated workbook and sheet metadata preservation evidence only',
				),
			}),
			expect.objectContaining({
				evidenceId: 'workbook-metadata-attribute-preservation-proof',
				acceptedScope: expect.stringMatching(/64f63423.*95f74c40.*2abf9a4e/),
				command: expect.stringContaining('preserves workbook and sheet protection'),
				boundary: expect.stringContaining(
					'fileVersion/fileSharing/sheet-view/page-setup/header-footer semantic interpretation',
				),
			}),
			expect.objectContaining({
				evidenceId: 'workbook-metadata-attribute-preservation-proof',
		...
		expect(index.qssLeapfrogReleaseMatrix.rows[1].acceptedEvidence).toEqual(
			expect.arrayContaining([
				...
				expect.objectContaining({
					...
					acceptedScope: expect.stringMatching(/1410d809.*c33695a7/),
					command: expect.stringContaining(
			...
			expect.arrayContaining([
				expect.objectContaining({
					...
					evidenceProvesIt: expect.arrayContaining([
						...
						expect.stringContaining('moveRange rewrites sheet autoFilter metadata'),
						expect.stringContaining(
							'commits public workbook strong-hash protection with honest metadata reporting',
						),
					]),
					forbiddenWording: expect.arrayContaining([
						...
						expect.stringContaining('filter criteria authoring'),
				...
				expect.objectContaining({
					...
					evidenceProvesIt: expect.arrayContaining([
						...
						expect.stringContaining('ISFORMULA spills formula masks'),
						...
						expect.stringContaining('rank functions spill over number and order arguments'),
						expect.stringContaining('top-level rank functions implicitly intersect'),
						expect.stringContaining('percentile and quartile functions spill over selector'),
						expect.stringContaining(
						...
						expect.stringContaining('forecast functions spill over target x values'),
						expect.stringContaining('top-level forecast functions implicitly intersect'),
						expect.stringContaining('FORECAST.ETS family spills over selector arguments'),
						expect.stringContaining('top-level FORECAST.ETS family implicitly intersects'),
						...
						expect.stringContaining(
							'depreciation financial scalar functions spill over range operands',
						),
						...
						expect.stringContaining('combinatorics scalar functions spill over range operands'),
					]),
					forbiddenWording: expect.arrayContaining([
