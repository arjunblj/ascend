...
function safeOpenQssEvidence(): readonly ReleaseProofQssAcceptedEvidenceItem[] {
	return [
		...
		},
		{
			...
			kind: 'test',
			command:
				'bun test packages/engine/src/operations.test.ts packages/sdk/src/advanced-filter-sparkline.test.ts packages/sdk/src/structural-auto-filter.test.ts -t "row and column shifts update sparkline source and location ranges|structural row edits shift sparkline ranges through SDK save and reopen|moveRange rewrites worksheet metadata formulas that reference the moved range|moveRange rejects defined names and worksheet metadata formulas with partial moved range refs|moveRange rewrites sparkline ranges through SDK save and reopen|moveRange relocates same-sheet advanced filters contained in the source range|range transfers fail closed for unsupported advanced filter copies and partial moves|moveRange rewrites advanced filters through SDK save and reopen|moveRange relocates same-sheet autoFilter and sortState contained in the source range|range transfers fail closed for unsupported autoFilter copies and partial moves|moveRange rewrites sheet autoFilter metadata through SDK save and reopen" --timeout 30000',
			path: 'packages/engine/src/operations.test.ts; packages/engine/src/structural/sheet-topology.ts; packages/engine/src/structural/formula-rewrite.ts; packages/sdk/src/advanced-filter-sparkline.test.ts; packages/sdk/src/structural-auto-filter.test.ts',
			acceptedScope:
				'Commit 8357e5de proves structural row/column edits shift sparkline group source ranges, location ranges, date-axis ranges, and individual sparkline refs, with SDK save/reopen coverage for a generated workbook containing advanced filters and sparklines. Commit 16eb2c43 proves moveRange rewrites sparkline group range, locationRange, dateAxisRange, and per-sparkline range/location refs, fails closed on partial moved sparkline refs before mutation, and preserves the moved ranges through SDK save and reopen. Commit f91587b9 proves same-sheet moveRange rewrites contained advanced filter refs, AutoFilter refs, sort-state refs, and sort-condition refs through SDK save/reopen while failing closed for advanced-filter copies, cross-sheet moves, and partial moves before mutation. Commit 2d02e2c3 proves same-sheet moveRange rewrites contained sheet autoFilter refs, embedded sort-state refs, sheet sortState refs, and sort-condition refs through SDK save/reopen while failing closed for sheet-filter copies, cross-sheet moves, and partial moves before mutation.',
			boundary:
		...
		},
		{
			...
			kind: 'test',
			command:
				'bun test packages/engine/src/operations.test.ts -t "row and column inserts reject chart source refs before shifting them out of grid|row and column inserts reject sparkline ranges before shifting them out of grid|row and column inserts reject edge cell metadata before shifting it out of grid|row and column inserts reject visual anchors before shifting them out of grid|row and column inserts reject hyperlink locations before shifting them out of grid|row and column inserts reject filter and table ranges before shifting them out of grid|row and column inserts reject formulas and defined names before shifting them out of grid|row and column inserts reject worksheet metadata formulas before shifting them out of grid|row and column inserts reject legacy comment anchors before shifting them out of grid" --timeout 30000',
			...
			acceptedScope:
				'Commits 4e58852c, e4335064, 36b927f9, ec1ae480, 7d01d067, a5d971f2, 70517573, f8e99b9d, and 969b2587 prove structural row/column inserts fail closed before mutation when chart source refs, sparkline ranges, comments/threaded comments/hyperlinks/row-column metadata, visual anchors, hyperlink locations, filter/table ranges, formulas/defined names, worksheet metadata formulas, or legacy comment anchors would be shifted outside Excel worksheet bounds.',
			boundary:
		...
		},
		{
			...
			kind: 'test',
			command:
				'bun test packages/engine/src/operations.test.ts -t "appendRows rejects table growth past the worksheet bounds|appendRows rejects protected table growth unless row insertion is allowed|appendRows rejects imported formula bindings under expanded table rows|appendRows rejects occupied worksheet cells below the table" --timeout 30000',
			path: 'packages/engine/src/operations.test.ts; packages/engine/src/operations/table-ops.ts',
			acceptedScope:
				'Commits d88b9104, 681086a9, and f569e13d prove appendRows rejects table growth past Excel worksheet bounds, protected-sheet table growth, and occupied target cells before mutating the table range or writing cells, while still allowing protected-table appends when row insertion is explicitly allowed.',
			boundary:
				'Local table append fail-closed evidence only; it does not prove public workbook table-growth parity, successful append save/reopen behavior, totals-row insert shifting, structured-reference updates, arbitrary table repair, full sheet-protection parity, occupied-target repair, or Excel UI-equivalent table expansion.',
		},
		{
		...
		},
		{
			...
			kind: 'test',
			command:
				'bun test packages/core/src/workbook.test.ts packages/io-xlsx/src/reader/reader.test.ts packages/io-xlsx/src/writer/writer.test.ts -t "clones sheet table metadata without aliasing nested refs|clones workbook settings and preserved metadata without aliasing|clones connection and external-link inventories without aliasing nested metadata|parses workbook views, workbook properties, and external references|parses workbook and sheet protection metadata|preserves workbook views and external reference wiring on round-trip|preserves workbook and sheet protection on round-trip|preserves unsupported workbook child XML when workbook metadata is regenerated" --timeout 30000',
			path: 'packages/core/src/sheet.ts; packages/core/src/workbook.ts; packages/core/src/workbook.test.ts; packages/io-xlsx/src/reader/workbook.ts; packages/io-xlsx/src/reader/reader.test.ts; packages/io-xlsx/src/writer/workbook.ts; packages/io-xlsx/src/writer/writer.test.ts',
			acceptedScope:
				'Commit 824e57c5 proves generated workbookView extra attributes such as minimized, showSheetTabs, and windowWidth are cloned, read, written, and reopened without being collapsed to the core workbook-view fields. Commit 64f63423 extends the same preservation shape to workbookPr extra attributes such as checkCompatibility and autoCompressPictures. Commit 95f74c40 preserves extra workbookProtection attributes such as futureProtectionMode, commit 2abf9a4e preserves extra calcPr attributes such as refMode and fullPrecision, commit 3e79423d preserves workbook fileVersion fields and extra attributes such as productRelease, commit aa94cbd0 preserves workbook fileSharing fields and extra attributes such as sharingMode, commit 1bd995e5 preserves unsupported workbook-level child XML when workbook metadata is regenerated, and commit 4d292666 preserves non-namespace workbook root attributes such as mc:Ignorable and xr:uid in that regenerated workbook.xml path. Commit 25d08758 proves workbook query-table field metadata and external-link sheet-name, defined-name, and DDE item inventories are deeply cloned instead of aliased. Commit 9cd32ae4 proves sheet view, tab color, outline, sheet format, drawing refs, page margins/setup/print options, and header/footer metadata are deeply cloned before mutation.',
			boundary:
				'Generated workbook and sheet metadata preservation evidence only; it does not prove public-workbook generality, full workbook-view parity, workbook protection/security behavior, Excel UI layout equivalence, password correctness, unsupported workbook child semantics, arbitrary workbookPr/workbookView/workbookProtection/calcPr/fileVersion/fileSharing/sheet-view/page-setup/header-footer semantic interpretation, or workbook root-attribute semantic interpretation.',
		},
		{
...
function packageActionQssEvidence(): readonly ReleaseProofQssAcceptedEvidenceItem[] {
	return [
		...
		},
		{
			...
			path: 'packages/core/src/connection.ts; packages/io-xlsx/src/reader/connections.ts; packages/io-xlsx/src/reader/connections.test.ts; packages/sdk/src/agent-workflow.ts; packages/sdk/src/connection-inventory.test.ts; packages/sdk/src/read-view.ts; packages/sdk/src/types.ts; fixtures/corpus/external-refresh-contract.test.ts',
			acceptedScope:
				'Commit caa08959 makes SDK post-write verification report reopened workbook/query-table connection metadata after an approved public query-table refresh metadata edit, with safe-open blocker coverage for the dangling-thumbnail query-table fixture. Commit 62566e09 expands reader, SDK inspect/refresh metadata, and post-write proof to report public connection type, description, deleted/background/keepAlive/interval flags, source file, command text, and a hasConnectionString boolean without exposing connection-string contents. Commits a7585a67, 5f7875c5, 8c127e93, 9ddcdc01, a8fef5e7, f0f075ac, and e55c94e6 add local reader/SDK proof for connection commandType/serverCommand, connection new/reconnection flags, query-table background/preserve/width/formula/edit/disable-refresh flags, query-table display/format flags, query-table refresh nextId and field mappings, and web-query url/htmlTables/xml/sourceData metadata surfacing through inspect, refreshMetadata, and postWrite.dataConnections.',
			boundary:
		...
		},
		{
			...
			acceptedScope:
				'Commits 1410d809, 8e351092, 75698a98, 346da8b1, 844336cb, 2700c72a, 33169e31, 9f22a961, 6370a2f2, 8e5b8422, 5a027510, c942f981, 9b66af81, 4c463d5e, 3d0d1754, 4a8acca1, 690a1642, c51f5f70, 12b26149, 8be0d0db, de7bd2bc, afcad38a, and c33695a7 classify package relationships, content-type sidecars, feature-derived owner scopes, and parts for active content/control/embedding/custom UI, Power Query/Data Model/model-table, chart/drawing/VML/image/style/color, document properties/thumbnails, custom XML/custom XML props/XML maps, diagram data, revision metadata, VBA/signature, calc-chain, sheet metadata, slicer/timeline, pivot, comments, threaded-comments/persons, shared-strings, styles, theme, table, chart-sheet, macro-sheet, connection, query-table, external-link, media, and printer-settings surfaces with owner scopes and preservation policies.',
			boundary:
	...
	return [
		{
			...
			evidenceProvesIt: [
				...
				'bun test packages/io-xlsx/src/reader/connections.test.ts packages/sdk/src/connection-inventory.test.ts fixtures/corpus/external-refresh-contract.test.ts -t "connection part inventory|connection SDK inventory|reports and preserves public query-table refresh surfaces without executing them|commit proof reports reopened public query-table connection metadata" --timeout 60000',
				'bun test packages/engine/src/operations.test.ts packages/io-xlsx/src/writer/writer.test.ts packages/sdk/src/ops-schema.test.ts packages/sdk/src/agent-workflow.test.ts -t "setConnectionRefresh|updates XML-legal single-quoted connection attributes without duplicating them|setConnectionRefresh is exposed with connection refresh guidance|commits public workbook connection scheduling edits through save and reopen" --timeout 30000',
				'bun test packages/io-xlsx/src/reader/external-links.test.ts packages/io-xlsx/src/package-graph.test.ts packages/io-xlsx/src/package-graph-fidelity.test.ts -t "parses DDE and OLE external link source metadata|inventories OLE external link relationship binding across save and reopen|inventories DDE external link item metadata across save and reopen|normalizes content types, relationship identity, owners, and feature families|uses OPC source, id, target, and TargetMode fields for adversarial relationship drift|classifies external workbook path relationships as external-link metadata" --timeout 60000',
				'bun test apps/cli/src/cli.test.ts -t "agent-init prints the canonical agent workflow contract" --timeout 30000',
			...
			allowedWording:
				'Ascend provides local runnable SDK, installed-SDK package-bin, CLI, API, and MCP safe-edit workflow examples with root commands, explicit trust-preflight steps, shared SDK proof-summary gates, workflow discovery that points agents to postWrite.dataConnections, postWrite.formulaState, and postWrite.visuals, generated safe-edit example output that surfaces reopened-output post-write proof fields for data connections, formula state, security, and visuals, public query-table refresh/data-connection metadata including source file, command text/type, server-command flag, web-query url/table/xml/source flags, query-table refresh/preserve/width/formula/edit/display/format flags, query-table refresh nextId and field mappings, refresh flags, and connection-string presence without secret disclosure, workbook connection scheduling metadata edits for backgroundRefresh, keepAlive, refreshInterval, refreshOnLoad, saveData, and refreshedVersion without connection execution, external-link source binding, DDE item metadata, and package relationship classification for inspection/preservation, chart-source/visual object reporting, local chart source refs shifted or rebased by structural row/column/range-move edits with reopened public-workbook proof, local sparkline source/location refs plus contained sheet-filter and advanced-filter refs shifted or rebased by structural row/column/range-move edits with SDK save/reopen proof, calc-chain/formula-cache reporting with cached and missing value locations, hidden-sheet/workbook-view topology, workbook structure-protection and strong-hash metadata reporting without raw hash/salt disclosure, protected-range metadata commit proof, and machine-readable proof context for generated/public workbooks.',
			forbiddenWording: [
				'Do not claim arbitrary workbook safety, package publication, registry download proof, hosted service readiness, public-workbook generality, connection execution, arbitrary connection authoring, credential safety, external-data freshness, external-link execution, DDE execution, linked-workbook fetch/validation, arbitrary broken-link repair, fresh recalculation, full drawing/layout parity, full chart authoring, chart layout editing, full sparkline rendering or arbitrary sparkline OOXML preservation, filter criteria authoring, copied/cross-sheet filter preservation, structured-reference chart rewriting, external workbook chart-reference rewriting, complete workbook-view parity, password correctness validation, raw protection hash/salt disclosure, protection enforcement/security validation, complete API package health, or complete workflow observability from the local generated/public-workbook examples.',
			],
			ownerLoop: 'release',
			nextOwnerAction:
		...
		},
		{
			...
			releaseOrSotaClaimBecameMoreTrue:
				'Ascend formula/calc behavior is more credible for dynamic-array-style workflows because common logical masks, IS predicates including formula-reference masks, ERROR.TYPE, SWITCH, IFS, ADDRESS, text scalar/extraction/search/replacement/comparison/value-conversion/formatting/slicing/text-conversion functions, common date/time/interval/workday scalar functions, and common math/engineering/radix/engineering-special/complex/financial/statistical/statistical-test/order-statistic/rank/percentile-quartile/percentrank/forecast/forecast-ets/trimmean/aggregate/subtotal/database-field plus legacy statistical alias/rounding/conversion/logarithmic/trigonometric/combinatorics scalar functions now map over range or array operands and spill through focused conditional contexts instead of collapsing to a top-left scalar, with local proof that scalar ADDRESS, text-conversion, date/workday, statistical-test/order-statistic/rank/percentile-quartile/percentrank/forecast/forecast-ets/trimmean/aggregate/subtotal/database-field selector, and engineering-special/complex calls implicitly intersect range arguments while preserving holiday-range arguments.',
			evidenceProvesIt: [
				'bun test packages/engine/src/calc.test.ts packages/formulas/src/functions/functions.test.ts -t "error predicates map over arrays inside IF conditions|NOT spills boolean masks for range operands in array formulas|common IS predicates spill boolean masks for range operands|ISFORMULA spills formula masks for range operands in array formulas|ERROR.TYPE spills error codes for range operands|IS predicates do not coerce text as numbers|SWITCH spills results for array expressions|SWITCH array expressions preserve lazy branch evaluation|IFS spills results for array conditions|IFS array conditions preserve selected array results and lazy branch evaluation|common text scalar functions spill over range operands in array formulas|text extraction functions spill over range operands in array formulas|FIND and SEARCH spill over range operands in array formulas|SUBSTITUTE and REPLACE spill over range operands in array formulas|EXACT, PROPER, and VALUE spill over range operands in array formulas|text conversion functions spill over selector arguments while preserving joined ranges|top-level text conversion functions implicitly intersect selector ranges|character code and repeat text functions spill over range operands in array formulas|numeric text formatting functions spill over range operands in array formulas|TEXTBEFORE and TEXTAFTER spill over range operands in array formulas|common date and time scalar functions spill over range operands in array formulas|date interval scalar functions spill over range operands in array formulas|workday scalar functions spill while preserving holiday range operands|date and workday scalar functions implicitly intersect date arguments and preserve holiday ranges|statistical distribution scalar functions spill over range operands in array formulas|statistical test functions spill over selector arguments while preserving data ranges|top-level statistical test functions implicitly intersect selector ranges|order statistic functions spill over array k arguments while preserving data ranges|top-level order statistic functions implicitly intersect range k arguments|rank functions spill over number and order arguments while preserving reference ranges|top-level rank functions implicitly intersect number and order ranges|percentile and quartile functions spill over selector arguments while preserving data ranges|top-level percentile and quartile functions implicitly intersect selector ranges|percentrank functions spill over lookup and significance arguments while preserving data ranges|top-level percentrank functions implicitly intersect lookup and significance ranges|forecast functions spill over target x values while preserving known ranges|top-level forecast functions implicitly intersect target x ranges|FORECAST.ETS family spills over selector arguments while preserving values and timelines|top-level FORECAST.ETS family implicitly intersects selector ranges|TRIMMEAN spills over percent arguments while preserving the data range|top-level TRIMMEAN implicitly intersects percent ranges|AGGREGATE selector functions spill over k while preserving the data range|top-level AGGREGATE selector functions implicitly intersect k ranges|AGGREGATE reference form preserves multiple range arguments instead of spilling them|top-level SUBTOTAL implicitly intersects function_num ranges and preserves reference ranges|database functions spill over field selectors while preserving database and criteria ranges|top-level database functions implicitly intersect field ranges|DGET returns one matching record and #NUM! for multiple matching records|DGET returns #NUM! when multiple records match|legacy statistical compatibility functions spill over range operands in array formulas|loan financial scalar functions spill over range operands in array formulas|financial helper scalar functions spill over range operands in array formulas|depreciation financial scalar functions spill over range operands in array formulas|common math scalar functions spill over range operands in array formulas|math conversion scalar functions spill over range operands in array formulas|engineering bitwise scalar functions spill over range operands in array formulas|engineering radix conversion functions spill over range operands in array formulas|engineering special and complex scalar functions spill over range operands in array formulas|engineering special and complex scalar functions implicitly intersect range operands|ADDRESS spills over range row and column arguments in array formulas|top-level ADDRESS implicitly intersects range row and column arguments|rounding scalar functions spill over range operands in array formulas|logarithmic scalar functions spill over range operands in array formulas|trigonometric scalar functions spill over range operands in array formulas|hyperbolic and reciprocal trig scalar functions spill over range operands in array formulas|combinatorics scalar functions spill over range operands in array formulas" --timeout 30000',
			],
			allowedWording:
				'Ascend has local regression proof for array/range mapping of NOT, ISERROR, ISERR, ISNA, ISBLANK, ISTEXT, ISLOGICAL, ISNONTEXT, ISEVEN, ISODD, ISFORMULA, ERROR.TYPE, SWITCH, IFS, ADDRESS, LEN, UPPER, LOWER, TRIM, LEFT, MID, RIGHT, FIND, SEARCH, SUBSTITUTE, REPLACE, EXACT, PROPER, VALUE, NUMBERVALUE, VALUETOTEXT, ARRAYTOTEXT, TEXTJOIN, CHAR, UNICHAR, CODE, UNICODE, CLEAN, REPT, TEXT, DOLLAR, FIXED, TEXTBEFORE, TEXTAFTER, YEAR, MONTH, DAY, EOMONTH, EDATE, HOUR, MINUTE, SECOND, DATEVALUE, TIMEVALUE, DATE, TIME, DAYS, WEEKDAY, DATEDIF, DAYS360, YEARFRAC, WEEKNUM, ISOWEEKNUM, NETWORKDAYS, NETWORKDAYS.INTL, WORKDAY, WORKDAY.INTL, PMT, FV, PV, NPER, RATE, IPMT, PPMT, SLN, SYD, DDB, DB, VDB, ISPMT, CUMIPMT, CUMPRINC, EFFECT, NOMINAL, PDURATION, RRI, DOLLARDE, DOLLARFR, NORM.DIST, NORM.INV, NORM.S.DIST, NORM.S.INV, BINOM.DIST, BINOM.INV, BINOM.DIST.RANGE, POISSON.DIST, EXPON.DIST, WEIBULL.DIST, GAMMA.DIST, GAMMA.INV, LOGNORM.DIST, LOGNORM.INV, NEGBINOM.DIST, HYPGEOM.DIST, BETA.DIST, BETA.INV, CHISQ.DIST, CHISQ.DIST.RT, CHISQ.INV, CHISQ.INV.RT, T.DIST, T.DIST.2T, T.DIST.RT, T.INV, T.INV.2T, F.DIST, F.DIST.RT, F.INV, F.INV.RT, FISHER, FISHERINV, STANDARDIZE, PHI, GAUSS, GAMMA, GAMMALN, GAMMALN.PRECISE, CONFIDENCE.NORM, CONFIDENCE.T, PROB, T.TEST, TTEST, Z.TEST, ZTEST, LARGE, SMALL, RANK, RANK.EQ, RANK.AVG, PERCENTILE, PERCENTILE.INC, PERCENTILE.EXC, QUARTILE, QUARTILE.INC, QUARTILE.EXC, PERCENTRANK, PERCENTRANK.INC, PERCENTRANK.EXC, FORECAST, FORECAST.LINEAR, FORECAST.ETS, FORECAST.ETS.CONFINT, FORECAST.ETS.STAT, FORECAST.ETS.SEASONALITY, TRIMMEAN, AGGREGATE, SUBTOTAL, DSUM, DAVERAGE, DCOUNT, DCOUNTA, DMAX, DMIN, DPRODUCT, DSTDEV, DSTDEVP, DVAR, DVARP, DGET, BETADIST, BETAINV, BINOMDIST, CRITBINOM, CHIDIST, CHIINV, CONFIDENCE, EXPONDIST, FDIST, FINV, GAMMADIST, GAMMAINV, HYPGEOMDIST, LOGINV, LOGNORMDIST, NEGBINOMDIST, NORMDIST, NORMINV, NORMSDIST, NORMSINV, POISSON, TDIST, TINV, WEIBULL, ABS, ROUND, MOD, POWER, SIGN, INT, ROUNDUP, ROUNDDOWN, TRUNC, CEILING, CEILING.MATH, CEILING.PRECISE, FLOOR, FLOOR.MATH, FLOOR.PRECISE, EVEN, ODD, MROUND, QUOTIENT, BASE, DECIMAL, ROMAN, ARABIC, SQRTPI, BITAND, BITOR, BITXOR, BITLSHIFT, BITRSHIFT, DELTA, GESTEP, BIN2DEC, BIN2HEX, BIN2OCT, DEC2BIN, DEC2HEX, DEC2OCT, HEX2BIN, HEX2DEC, HEX2OCT, OCT2BIN, OCT2DEC, OCT2HEX, CONVERT, ERF, ERF.PRECISE, ERFC, ERFC.PRECISE, BESSELI, BESSELJ, BESSELK, BESSELY, COMPLEX, IMREAL, IMAGINARY, IMABS, IMARGUMENT, IMCONJUGATE, IMSUB, IMDIV, IMPOWER, IMSQRT, IMEXP, IMLN, IMSIN, IMCOS, IMLOG10, IMLOG2, IMTAN, IMSINH, IMCOSH, IMSEC, IMCSC, IMCOT, IMSECH, IMCSCH, LOG10, LOG, LN, EXP, SIN, COS, TAN, ASIN, ACOS, ATAN, ATAN2, RADIANS, DEGREES, SINH, COSH, TANH, COT, SEC, CSC, COTH, SECH, CSCH, ACOT, ACOSH, ASINH, ATANH, ACOTH, FACT, FACTDOUBLE, COMBIN, and COMBINA in focused formula-engine cases, including spilled IF/IFERROR/SWITCH/IFS masks, lazy branch handling, scalar ADDRESS/text-conversion/date/workday/statistical-test/order-statistic/rank/percentile-quartile/percentrank/forecast/forecast-ets/trimmean/aggregate/subtotal/database-field implicit intersection, and scalar engineering-special/complex implicit intersection for range arguments while preserving holiday ranges.',
			forbiddenWording: [
...
const AGENT_WORKFLOW_OBSERVABILITY_BLOCKER = {
	...
	evidenceWeHave: [
		...
		'Post-write data-connection proof is accepted after `caa08959 fix(sdk): report post-write data connections`, `62566e09 fix(sdk): report connection proof metadata`, `a7585a67 fix(io-xlsx): inventory connection command metadata`, `5f7875c5 fix(io-xlsx): inventory query table refresh metadata`, `8c127e93 fix(io-xlsx): inventory web query connection metadata`, `9ddcdc01 fix(sdk): expose connection command metadata`, and `1cb093fe feat(sdk): edit workbook connection scheduling metadata`: `bun test packages/io-xlsx/src/reader/connections.test.ts packages/sdk/src/connection-inventory.test.ts fixtures/corpus/external-refresh-contract.test.ts -t "connection part inventory|connection SDK inventory|reports and preserves public query-table refresh surfaces without executing them|commit proof reports reopened public query-table connection metadata" --timeout 60000` and `bun test packages/sdk/src/agent-workflow.test.ts -t "commits public workbook connection scheduling edits through save and reopen" --timeout 30000` prove approved public query-table/data-connection commits return reopened connection counts, states, names, ids, part paths, type/description/source/command commandType/serverCommand/web-query flags, query-table refresh/preserve/width/formula/edit flags, `hasConnectionString`, and workbook connection scheduling metadata edits without exposing connection-string contents or executing connections in `postWrite.dataConnections`.',
