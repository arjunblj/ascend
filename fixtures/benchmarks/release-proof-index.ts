...
function safeOpenQssEvidence(): readonly ReleaseProofQssAcceptedEvidenceItem[] {
	return [
		...
		},
		{
			...
			kind: 'test',
			command:
				'bun test packages/core/src/workbook.test.ts packages/io-xlsx/src/reader/reader.test.ts packages/io-xlsx/src/writer/writer.test.ts -t "clones workbook settings and preserved metadata without aliasing|parses workbook views, workbook properties, and external references|parses workbook and sheet protection metadata|preserves workbook views and external reference wiring on round-trip|preserves workbook and sheet protection on round-trip|preserves unsupported workbook child XML when workbook metadata is regenerated" --timeout 30000',
