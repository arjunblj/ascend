...
function todayCommitClaimMatrix(): readonly ReleaseProofTodayCommitClaimMatrixRow[] {
	return [
		...
		},
		{
			...
			releaseOrSotaClaimBecameMoreTrue:
				'Ascend formula/calc behavior is more credible for dynamic-array-style workflows because common IS predicates, ERROR.TYPE, SWITCH, IFS, text scalar/extraction/search/replacement/comparison/value-conversion functions, and common math/rounding/logarithmic/trigonometric scalar functions now map over range or array operands and spill through focused conditional contexts instead of collapsing to a top-left scalar.',
			evidenceProvesIt: [
				'bun test packages/engine/src/calc.test.ts packages/formulas/src/functions/functions.test.ts -t "error predicates map over arrays inside IF conditions|common IS predicates spill boolean masks for range operands|ERROR.TYPE spills error codes for range operands|IS predicates do not coerce text as numbers|SWITCH spills results for array expressions|SWITCH array expressions preserve lazy branch evaluation|IFS spills results for array conditions|IFS array conditions preserve selected array results and lazy branch evaluation|common text scalar functions spill over range operands in array formulas|text extraction functions spill over range operands in array formulas|FIND and SEARCH spill over range operands in array formulas|SUBSTITUTE and REPLACE spill over range operands in array formulas|EXACT, PROPER, and VALUE spill over range operands in array formulas|common math scalar functions spill over range operands in array formulas|rounding scalar functions spill over range operands in array formulas|logarithmic scalar functions spill over range operands in array formulas|trigonometric scalar functions spill over range operands in array formulas|hyperbolic and reciprocal trig scalar functions spill over range operands in array formulas" --timeout 30000',
			],
			allowedWording:
				'Ascend has local regression proof for array/range mapping of ISERROR, ISERR, ISNA, ISBLANK, ISTEXT, ISLOGICAL, ISNONTEXT, ISEVEN, ISODD, ERROR.TYPE, SWITCH, IFS, LEN, UPPER, LOWER, TRIM, LEFT, MID, RIGHT, FIND, SEARCH, SUBSTITUTE, REPLACE, EXACT, PROPER, VALUE, ABS, ROUND, MOD, POWER, SIGN, INT, ROUNDUP, ROUNDDOWN, TRUNC, CEILING, CEILING.MATH, CEILING.PRECISE, FLOOR, FLOOR.MATH, FLOOR.PRECISE, EVEN, ODD, LOG10, LOG, LN, EXP, SIN, COS, TAN, ASIN, ACOS, ATAN, ATAN2, RADIANS, DEGREES, SINH, COSH, TANH, COT, SEC, CSC, COTH, SECH, CSCH, ACOT, ACOSH, ASINH, ATANH, and ACOTH in focused formula-engine cases, including spilled IF/IFERROR/SWITCH/IFS masks and lazy branch handling.',
			forbiddenWording: [
