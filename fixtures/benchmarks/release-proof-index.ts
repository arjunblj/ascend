...
function safeOpenQssEvidence(): readonly ReleaseProofQssAcceptedEvidenceItem[] {
	return [
		...
		},
		{
			...
			kind: 'test',
			command:
				'bun test apps/cli/src/cli.test.ts apps/cli/src/file-errors.test.ts -t "custom UI callbacks|open-plan --json reports missing files|raw ENOENT noise|missing ops sidecar|structured missing workflow input guidance|structured missing workbook guidance|verification commands --json return structured missing input guidance|read surfaces --json return structured missing input guidance|proof and recovery commands --json return structured missing input guidance|calc and export --json return structured missing input guidance|direct mutation commands --json return structured missing input guidance before opening files|utility commands --json return structured missing input guidance|legacy string cli errors still return coded JSON failures|formula edit commands --json return structured missing input guidance" --timeout 30000',
