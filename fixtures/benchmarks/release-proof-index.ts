...
function safeOpenQssEvidence(): readonly ReleaseProofQssAcceptedEvidenceItem[] {
	return [
		...
		},
		{
			...
			acceptedScope:
				'Commits 215d6e57, 7c1a9708, and 8ce0fbe2 return structured retryable missing-workbook-reference errors for API plan/commit/open-plan/inspect/read/agent-view workflow requests instead of generic missing-file responses.',
			boundary:
...
const AGENT_WORKFLOW_OBSERVABILITY_BLOCKER = {
	...
	evidenceMissing: [
		...
		'Owner-approved decision that prepared-plan rollback/retry evidence is enough for recovery plumbing and should not continue as low-value API/SDK/MCP work without a new public workflow failure class.',
