...
function packageActionQssEvidence(): readonly ReleaseProofQssAcceptedEvidenceItem[] {
	return [
		...
		},
		{
			...
			acceptedScope:
				'Commits 1410d809, 8e351092, 75698a98, 346da8b1, 844336cb, 2700c72a, 33169e31, 9f22a961, 6370a2f2, 8e5b8422, 5a027510, c942f981, 9b66af81, 4c463d5e, 3d0d1754, 4a8acca1, 690a1642, c51f5f70, 12b26149, and 8be0d0db classify package relationships, content-type sidecars, and parts for active content/control/embedding/custom UI, Power Query/Data Model/model-table, chart/drawing/VML/image/style/color, document properties/thumbnails, custom XML/custom XML props/XML maps, diagram data, revision metadata, VBA/signature, calc-chain, sheet metadata, slicer/timeline, comments, threaded-comments, shared-strings, styles, theme, chart-sheet, macro-sheet, connection, query-table, and external-link surfaces with owner scopes and preservation policies.',
			boundary:
				'Package relationship classification evidence only; it does not execute active content, refresh Power Query or connections, understand Data Model semantics, edit custom UI or custom XML, render or edit drawings/images/charts, edit chart styles/colors, refresh slicers/timelines, author comments, execute or validate macro-sheet logic, prove shared-string generation equivalence, prove style/theme semantics, prove document-property or calc metadata freshness, validate external-link contents, verify signatures, or guarantee arbitrary unknown-part recovery.',
		},
		{
...
function todayCommitClaimMatrix(): readonly ReleaseProofTodayCommitClaimMatrixRow[] {
	return [
		...
		},
		{
			...
			releaseOrSotaClaimBecameMoreTrue:
				'Ascend formula/calc behavior is more credible for dynamic-array-style workflows because common IS predicates, ERROR.TYPE, SWITCH, IFS, text scalar/extraction/search/replacement/comparison/value-conversion functions, and common math/rounding/logarithmic scalar functions now map over range or array operands and spill through focused conditional contexts instead of collapsing to a top-left scalar.',
			evidenceProvesIt: [
				'bun test packages/engine/src/calc.test.ts packages/formulas/src/functions/functions.test.ts -t "error predicates map over arrays inside IF conditions|common IS predicates spill boolean masks for range operands|ERROR.TYPE spills error codes for range operands|IS predicates do not coerce text as numbers|SWITCH spills results for array expressions|SWITCH array expressions preserve lazy branch evaluation|IFS spills results for array conditions|IFS array conditions preserve selected array results and lazy branch evaluation|common text scalar functions spill over range operands in array formulas|text extraction functions spill over range operands in array formulas|FIND and SEARCH spill over range operands in array formulas|SUBSTITUTE and REPLACE spill over range operands in array formulas|EXACT, PROPER, and VALUE spill over range operands in array formulas|common math scalar functions spill over range operands in array formulas|rounding scalar functions spill over range operands in array formulas|logarithmic scalar functions spill over range operands in array formulas" --timeout 30000',
			],
			allowedWording:
				'Ascend has local regression proof for array/range mapping of ISERROR, ISERR, ISNA, ISBLANK, ISTEXT, ISLOGICAL, ISNONTEXT, ISEVEN, ISODD, ERROR.TYPE, SWITCH, IFS, LEN, UPPER, LOWER, TRIM, LEFT, MID, RIGHT, FIND, SEARCH, SUBSTITUTE, REPLACE, EXACT, PROPER, VALUE, ABS, ROUND, MOD, POWER, SIGN, INT, ROUNDUP, ROUNDDOWN, TRUNC, CEILING, CEILING.MATH, CEILING.PRECISE, FLOOR, FLOOR.MATH, FLOOR.PRECISE, EVEN, ODD, LOG10, LOG, LN, and EXP in focused formula-engine cases, including spilled IF/IFERROR/SWITCH/IFS masks and lazy branch handling.',
			forbiddenWording: [
