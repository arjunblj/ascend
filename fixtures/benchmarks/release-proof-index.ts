...
function safeOpenQssEvidence(): readonly ReleaseProofQssAcceptedEvidenceItem[] {
	return [
		...
		},
		{
			...
			kind: 'test',
			command:
				'bun test packages/engine/src/operations.test.ts packages/sdk/src/advanced-filter-sparkline.test.ts -t "row and column shifts update sparkline source and location ranges|structural row edits shift sparkline ranges through SDK save and reopen|moveRange rewrites worksheet metadata formulas that reference the moved range|moveRange rejects defined names and worksheet metadata formulas with partial moved range refs|moveRange rewrites sparkline ranges through SDK save and reopen|moveRange relocates same-sheet advanced filters contained in the source range|range transfers fail closed for unsupported advanced filter copies and partial moves|moveRange rewrites advanced filters through SDK save and reopen" --timeout 30000',
			path: 'packages/engine/src/operations.test.ts; packages/engine/src/structural/sheet-topology.ts; packages/engine/src/structural/formula-rewrite.ts; packages/sdk/src/advanced-filter-sparkline.test.ts',
			acceptedScope:
			...
			boundary:
				'Sparkline and advanced-filter structural edit evidence only; it does not prove Excel rendering parity, arbitrary sparkline OOXML preservation, unsupported sparkline settings, public workbook generality, advanced-filter criteria authoring, cross-sheet advanced-filter moves, or copied advanced-filter preservation.',
		},
		{
