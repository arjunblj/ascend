...
async function directOrderedReadAssertions(
	args: Args,
	bytes: Uint8Array | undefined,
): Promise<Record<string, string | number | boolean | null> | undefined> {
	const sourceBytes = bytes ?? (await Bun.file(args.file).bytes())
...
function orderedSheetScanAssertions(
	...
	const usedRange =
		state.cellCount === 0
			? `${sheetName}!empty`
			: `${sheetName}!${indexToColumn(state.minCol)}${state.minRow + 1}:${indexToColumn(state.maxCol)}${state.maxRow + 1}`
	const physicalUsedRange =
		state.physicalCellCount === 0
			? `${sheetName}!empty`
			: `${sheetName}!${indexToColumn(state.physicalMinCol)}${state.physicalMinRow + 1}:${indexToColumn(
					state.physicalMaxCol,
				)}${state.physicalMaxRow + 1}`
	const usedRanges = [usedRange]
...
function scanCanonicalOrderedValuesCell(
	...
	const ref = `${state.sheetName}!${indexToColumn(parsed.col)}${rowText}`
...
function parseExpectedCanonicalCellRef(
	...
	const rowStart = start + colText.length
...
function scanOrderedValuesCell(
	...
	if (formula !== undefined) {
		...
		const ref = `${state.sheetName}!${indexToColumn(col)}${row + 1}`
...
function trackSemanticCell(state: OrderedSheetScanState, row: number, col: number): void {
	...
	state.minCol = Math.min(state.minCol, col)
	state.maxRow = Math.max(state.maxRow, row)
	state.maxCol = Math.max(state.maxCol, col)
}

function trackPhysicalCell(state: OrderedSheetScanState, row: number, col: number): void {
	state.physicalCellCount += 1
	state.physicalMinRow = Math.min(state.physicalMinRow, row)
	state.physicalMinCol = Math.min(state.physicalMinCol, col)
	state.physicalMaxRow = Math.max(state.physicalMaxRow, row)
	state.physicalMaxCol = Math.max(state.physicalMaxCol, col)
}
