...
function applyAxisShift(
	...
	const deletedTableColumnBlocker =
		axis === 'col' && delta < 0 ? findDeletedTableColumnReference(workbook, sheet, at, count) : null
...
interface DeletedTableColumnSet {
...
interface DeletedTableColumnReference {
...
function findDeletedTableColumnReference(
	workbook: Workbook,
	targetSheet: Sheet,
	at: number,
	count: number,
): DeletedTableColumnReference | null {
	const deletedSets = collectDeletedTableColumns(targetSheet, at, count)
	if (deletedSets.length === 0) return null

	for (const sheet of workbook.sheets) {
		for (const [row, col, cell] of sheet.cells.iterate()) {
			if (cell.formula === null) continue
			if (sheet === targetSheet && col >= at && col < at + count) continue
	...
	for (const sheet of workbook.sheets) {
		for (const table of sheet.tables) {
			...
			const deletedTableColumns =
				sheet === targetSheet ? deletedSets.find((set) => set.table === table) : undefined
			for (const [index, column] of table.columns.entries()) {
				if (deletedTableColumns?.deletedIndexes.has(index)) {
					continue
				}
				const formulas = [
...
function collectDeletedTableColumns(
	sheet: Sheet,
	at: number,
	count: number,
