...
export const EXCEL_CAPABILITIES: readonly ExcelCapability[] = [
	...
	cap(
		...
		'Pivot layout/style/options, extension layout defaults, output row/column item coordinates, output format areas, PivotChart format bindings, data-field display calculations, cache source identity, bounded or explicit full cache-record materialization, empty-output, page-filtered including multi-select page filters, sum/count/average saved-output audits including grouped count fields, and data-fields-on-rows saved-output audits, shared-item type/bounds/date-range group metadata, page-filter selections, and field item visibility/detail flags are inspectable; cache source and filter item edits are safely editable with refresh-on-open markers while full pivot management and output recalculation remain incomplete.',
		[
