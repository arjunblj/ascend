...
export const EXCEL_CAPABILITIES: readonly ExcelCapability[] = [
	...
	cap(
		...
		'editable',
		'P1',
		'Broaden docProps editing to preserve unknown typed-vector app-property structure while applying targeted edits.',
		'WorkbookPr properties are inspectable and editable through setWorkbookProperties with merge/replace and null-clearing semantics; package document properties are inspectable and editable through setDocumentProperties for core, app, and custom docProps parts while preserving untouched packages exactly.',
		[
	...
	cap(
		...
		'editable',
		'P0',
		'Add cross-workbook fixture checks and formula token rewrite planning.',
		'External link targets can be rewritten safely; cell, defined-name, chart-series, table, conditional-format, data-validation, sparkline, and x14 metadata formulas are audited before rewrites.',
		[
	...
	cap(
		...
		'editable',
		'P1',
		'Expand hyperlink rewrite coverage with mixed internal/external real-workbook fixtures.',
		'External URL links, internal workbook locations, display text, and tooltips are editable and round-trip through XLSX.',
	),
	cap(
		'sheets.merged-cells',
		'sheets/ranges',
		...
		'editable',
		'P1',
		'Broaden real-workbook fixtures for merge edits around sorted/filtered/table ranges.',
		'Merge operations validate overlapping ranges, and copy/move all/formats/styles modes transfer merged-cell layout with partial-merge guards.',
	),
	cap(
		'sheets.copy-paste',
		'sheets/ranges',
	...
	cap(
		...
		'inspectable',
		'P0',
		'Add calculated pivot output refresh after broadening multi-axis fixture coverage.',
		'Pivot layout/style/options, extension layout defaults, output row/column item coordinates, output format areas, PivotChart format bindings, data-field display calculations, cache source identity, bounded or explicit full cache-record materialization, empty-output, page-filtered including multi-select and grouped page filters, sum/count/average saved-output audits including grouped count fields, one-row column-field saved-output audits, percent-of-row saved-output audits, grouped month-axis saved-output audits, grouped multi-row saved-output audits with column fields, data-fields-on-columns saved-output audits, and data-fields-on-rows saved-output audits, shared-item type/bounds/date-range group metadata, page-filter selections, and field item visibility/detail flags are inspectable; cache source and filter item edits are safely editable with refresh-on-open markers while full pivot management and output recalculation remain incomplete.',
		[
			...
			'packages/engine/src/operations.test.ts',
			'packages/sdk/src/ops-schema.test.ts',
			'packages/sdk/src/pivot-output-audit.test.ts',
