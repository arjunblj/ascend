...
describe('interactive client contract', () => {
	...
	test('sortRange journals mark imported row definitions lossy', () => {
		const wb = AscendWorkbook.create()
		wb.apply([
			{
				op: 'setCells',
		...
		sheet.rowDefs.set(2, { collapsed: true, customHeight: false })
	...
	test('sortRange exact journals restore row-scoped data validations', () => {
		const wb = AscendWorkbook.create()
		wb.apply([
			{
				op: 'setCells',
		...
		expect(wb.sheet('Sheet1')?.dataValidations).toEqual([
			expect.objectContaining({ sqref: 'A2', formula1: 'B2' }),
			expect.objectContaining({ sqref: 'A3', formula1: 'B3' }),
		])
	...
	test('sortRange data validation journals mark non-suffix order lossy', () => {
		const wb = AscendWorkbook.create()
		wb.apply([
			{
				op: 'setCells',
	...
	test('journals expose lossy escaped dynamic spill member preimages', () => {
		...
		const changed = wb.apply(
			[{ op: 'setCells', sheet: "Bob's Budget", updates: [{ ref: 'A1', value: 99 }] }],
			{ journal: true },
		)
	...
	test('journals expose dynamic spill anchor preimages with case-insensitive spill anchor refs', () => {
		const wb = AscendWorkbook.create()
		const sheet = wb.getWorkbookModel().getSheet('Sheet1')
		if (!sheet) throw new Error('missing sheet')
		sheet.cells.set(0, 0, {
