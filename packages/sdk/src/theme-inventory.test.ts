...
describe('theme SDK inventory', () => {
	test('inspect exposes theme palette and font metadata', async () => {
		const wb = await AscendWorkbook.open(themeWorkbook())
		const info = wb.inspect()

		expect(info.themeSummary).toMatchObject({
			hasThemePart: true,
			name: 'Office Twist',
		...
		expect(info.themeSummary.colors).toEqual([
