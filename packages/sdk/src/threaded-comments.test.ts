...
describe('threaded comment SDK inventory', () => {
	test('inspect exposes threaded comment count and thread metadata', async () => {
		const wb = await AscendWorkbook.open(threadedCommentWorkbook())
		const info = wb.inspect()
		const sheet = wb.inspectSheet('Data')

		expect(info.threadedCommentCount).toBe(2)
		expect(info.sheets[0]?.threadedCommentCount).toBe(2)
		...
		expect(
			info.compatibility.features.find(
