...
describe('checker', () => {
	...
	test('detects shared formula members outside the master range', () => {
		...
		const issue = result.issues.find(
			(entry) => entry.details?.kind === 'shared-formula-member-outside-master-range',
		)
		expect(result.passed).toBe(false)
		expect(issue).toMatchObject({
	...
	test('detects shared formula members with hidden formula text', () => {
		const wb = createWorkbook()
		const s = wb.addSheet('Sheet1')
		s.cells.set(0, 0, {
			value: numberValue(2),
		...
		const result = check(wb)
		const issue = result.issues.find(
			(entry) => entry.details?.kind === 'shared-formula-member-formula-text-mismatch',
		)
		expect(result.passed).toBe(false)
		expect(issue).toMatchObject({
			rule: 'formula-binding-integrity',
			severity: 'error',
			message:
				'Shared formula member at Sheet1!A2 has formula text that would be hidden by shared metadata when saved',
			refs: ['Sheet1!A2', 'Sheet1!A1'],
			details: {
	...
	test('detects occupied cells detached inside a shared formula range', () => {
		const wb = createWorkbook()
		const s = wb.addSheet('Sheet1')
		s.cells.set(0, 0, {
			value: numberValue(2),
		...
		const result = check(wb)
		const issue = result.issues.find(
			(entry) => entry.details?.kind === 'shared-formula-range-member-mismatch',
		)
		expect(result.passed).toBe(false)
		expect(issue).toMatchObject({
			rule: 'formula-binding-integrity',
			severity: 'error',
			message:
				'Shared formula metadata for Sheet1!A1 has an inconsistent occupied cell inside its range',
	...
	test('accepts overlapping shared formula master ranges from real workbooks', () => {
		const wb = createWorkbook()
		const s = wb.addSheet('Sheet1')
		s.cells.set(0, 0, {
			value: numberValue(2),
		...
		s.cells.set(3, 0, {
			...
			formula: null,
			styleId: SID,
			formulaInfo: { kind: 'shared', sharedIndex: '1', isMaster: false, masterRef: 'A3' },
		})
	...
	test('detects spill sibling range drift', () => {
		const wb = createWorkbook()
		const s = wb.addSheet('Sheet1')
		s.cells.set(0, 0, {
			value: numberValue(1),
		...
		s.cells.set(2, 0, {
			...
			formula: null,
			styleId: SID,
			formulaInfo: { kind: 'spill', anchorRef: 'Sheet1!A1', ref: 'A1:A4', isAnchor: false },
		})

		const result = check(wb)
		const issue = result.issues.find(
			(entry) => entry.details?.kind === 'spill-sibling-range-mismatch',
		)
		expect(result.passed).toBe(false)
		expect(issue).toMatchObject({
			rule: 'formula-binding-integrity',
			severity: 'error',
			message: 'Spill metadata for Sheet1!A1 has inconsistent sibling ranges',
			refs: ['Sheet1!A2', 'Sheet1!A3'],
			details: {
				kind: 'spill-sibling-range-mismatch',
				anchorRef: 'Sheet1!A1',
				ranges: [
					{ ref: 'Sheet1!A2', kind: 'spill', range: 'A1:A3' },
					{ ref: 'Sheet1!A3', kind: 'spill', range: 'A1:A4' },
				],
			},
		})
	})
	...
	test('detects blocked spill sibling range drift', () => {
		...
		const result = check(wb)
		const issue = result.issues.find(
			(entry) => entry.details?.kind === 'blockedSpill-sibling-range-mismatch',
		)
		expect(result.passed).toBe(false)
		expect(issue).toMatchObject({
			rule: 'formula-binding-integrity',
			...
			details: {
				kind: 'blockedSpill-sibling-range-mismatch',
				anchorRef: 'Sheet1!A1',
				ranges: [
					{ ref: 'Sheet1!A1', kind: 'blockedSpill', range: 'A1:A3' },
					{ ref: 'Sheet1!A2', kind: 'blockedSpill', range: 'A1:A4' },
				],
			},
		})
	})
