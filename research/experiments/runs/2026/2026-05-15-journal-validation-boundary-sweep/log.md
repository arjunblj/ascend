...
The highest-value correctness work is not adding new mutation surfaces; it is making existing journal proof refuse invalid operation shapes consistently. Workbook metadata, formula syntax, pivot cache selectors, defined-name deletion, sheet-name creation, and table-name creation are likely boundary cases because their public operation shapes include optional selectors, editable fields, or Excel naming constraints.
...
All passed. `journal-exactness.test.ts` now has explicit boundary cases for rejected workbook metadata, invalid formulas, rejected pivot cache edits, missing defined-name deletes, invalid target sheet names, and invalid target table names.
