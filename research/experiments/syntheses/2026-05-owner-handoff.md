...
Streaming proof note: the release index now emits `streamingMatrixEvidence` for the package-action performance gate. Current proof covers three streaming cases, `docprops-passthrough`, `add-sheet-part`, and `calc-chain-drop`, with action kinds `passthrough`, `regenerate`, `add`, and `drop`; it explicitly leaves `error`, public macro, and public chart cases unproven for streaming. The gate remains owner-approval required.
...
| `package-action-proof` | `921dc41d423632c935741dd4fb4e7def4e1c2719c90263c13309e64093419803` | false | cases=8, passthrough=32, regenerate=40, add=3, drop=3, error=1, streamingProofCases=3 |
