...
Release proof index status: `fixtures/benchmarks/release-proof-index.ts` now lists the exact reproduction command, publication blockers, and owner-loop `readyWhen` gates for this artifact. The package-action artifact remains `needs-release-packaging` because synthetic edge packages must stay disclosed unless replaced by public binary fixtures, and this proof is local evidence rather than signed provenance or third-party attestation. The current no-timings stable shape digest is `60da8baa4a897e7edbd3f02fcb1a7026643bc68de1f5dedec082e71d29f03213`.
