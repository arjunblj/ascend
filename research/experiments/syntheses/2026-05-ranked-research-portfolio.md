...
| 9 | Columnar scan sidecars | Ascend can accelerate repeated table/range scans with disposable generation-keyed sidecars. | Real-world performance without replacing workbook truth. | Public real workbook table benchmarks with checksum parity, payload bytes, and generation invalidation. | Kill product promotion if build+invalidation cost erases repeated-scan gains or checksum parity fails. | Performance |
