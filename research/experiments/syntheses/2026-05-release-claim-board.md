...
- Package-action streaming boundary now shows `streamingProofCases=2` and `streamingRegenerateParts=1`; allowed wording remains representative streaming proofs for passthrough/regenerate/add only, not full streaming parity across drop/error or macro/chart cases.
...
- `streamingMatrixEvidence` now records the concrete streaming boundary in owner-handoff JSON: two representative streaming proof cases, covered action kinds `passthrough`, `regenerate`, and `add`, missing action kinds `drop` and `error`, and public non-streaming cases `calc-chain-drop`, `macro-passthrough`, and `chart-sidecar-accounting`. This is proof of the boundary, not satisfaction of `streaming-matrix-boundary`.
...
| Streaming writer | `docprops-passthrough` and `add-sheet-part` have representative streaming proofs covering passthrough/regenerate/add with passthrough-byte equality and no streaming issues. | "Representative streaming writer package-action proofs cover passthrough/regenerate/add." | "Full streaming parity across all package-action scenarios." |
