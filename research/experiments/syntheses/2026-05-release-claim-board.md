...
| **Auditable package-part mutation:** Ascend can explain write outcomes with structured package action evidence and rollback-journal issue reasons. | Fixture: docProps passthrough, generated sheet XML, calc-chain drop, signature invalidation, macro/ActiveX preservation, drawing/chart sidecar passthrough, unknown part rejection. Benchmark: package proof overhead in ms and bytes. API/CLI/MCP: compact summaries plus SDK full proof through existing options. Validation: plan/commit/reopen/diff/audit, package graph checks, journal v1 compatibility, typecheck, Biome, changed tests. Current checkpoint: commit-local proofs now infer source graph evidence from retained source bytes, so added package parts can be distinguished from regenerated existing parts. Competitor contrast: openpyxl and SheetJS expose preservation boundaries; Ascend's stronger claim is per-part accounting. Boundary: not signed provenance or Excel semantic refresh proof. | **Correctness/product.** Top handoff. |
