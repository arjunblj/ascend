...
- The only generated edge packages remaining in the package-action owner gate are `signature-invalidation-drop` and `unknown-part-error`; `docprops-passthrough` and `calc-chain-drop` are public fixture evidence.
- Formula rename remains frozen.
...
- Package-action proof covers 8 cases with 4 public fixtures, 2 generated workbooks, 2 generated edge packages, action totals `passthrough=32`, `regenerate=40`, `add=3`, `drop=3`, `error=1`, 8 source-graph evidence cases, 8 package-preservation journal issue cases, and 3 representative streaming proof cases.
...
- Package-action proof covers 8 cases with 4 public fixtures, 2 generated workbooks, 2 generated edge packages, action totals `passthrough=32`, `regenerate=40`, `add=3`, `drop=3`, `error=1`, and stable shape `921dc41d423632c935741dd4fb4e7def4e1c2719c90263c13309e64093419803`.
...
- Package-action compact proof covers all five action classes with 8 cases, 4 public fixtures, 2 generated workbooks, 2 generated edge packages, action totals `passthrough=32`, `regenerate=40`, `add=3`, `drop=3`, `error=1`, three representative streaming proofs, and stable shape `921dc41d423632c935741dd4fb4e7def4e1c2719c90263c13309e64093419803`. Generated edge-package policy, unsupported-feature boundaries, provenance wording, and streaming-matrix wording remain owner gates.
...
- Package-action streaming boundary now shows `streamingProofCases=3` and `streamingRegenerateParts=2`; allowed wording remains representative streaming proofs for passthrough/regenerate/add/drop only, not full streaming parity across error or macro/chart cases.
...
| Auditable package-part mutation | `package-action-proof` command, stable shape digest `b4cf0755f2fd40ff577ab87b6600ee14a58bbd69608796c903759e7e8b4d29e8`, all five action classes, representative streaming writer proofs, journal issue refs, post-write audit status, validation commands, and accepted or resolved `readyWhen` gates | Do not publish headline copy if synthetic edge packages are hidden, if chart XML is called byte-passthrough, if representative streaming proofs are described as full streaming matrix coverage, or if the proof implies SLSA/in-toto/signed provenance |
...
| Streaming writer | `docprops-passthrough`, `add-sheet-part`, and `calc-chain-drop` have representative streaming proofs covering passthrough/regenerate/add/drop with passthrough-byte equality and no streaming issues. | "Representative streaming writer package-action proofs cover passthrough/regenerate/add/drop." | "Full streaming parity across all package-action scenarios." |
...
Package-action owner acceptance checklist: current proof has 8 cases: 4 public fixtures, 2 generated workbook cases, and 2 generated edge-package cases. Disclosed generated structural edge cases are `signature-invalidation-drop` and `unknown-part-error`; all five action classes are present with `passthrough=32`, `regenerate=40`, `add=3`, `drop=3`, and `error=1`; source graph evidence and journal package issues appear in every case; `unknown-part-error` is the only post-write audit failure; three representative streaming proofs exist. Product must accept or replace remaining generated edge packages. Correctness must approve unsupported-feature boundaries and journal/package issue compatibility. Performance must accept narrow streaming wording or expand the matrix. Release must approve non-provenance wording and compact report publication policy. Until those checkboxes are complete, `package-action-proof` remains `headlineClaimAllowed=false`.
...
Package-action edge fixture policy audit: the tracked-corpus ZIP-entry scan covers 223 git-tracked XLSX/XLSM fixtures and shows partial public replacement potential, not full replacement. The corpus has 52 workbooks with `xl/calcChain.xml`, 16 with `docProps/custom.xml`, 4 with `customXml/` parts, 2 with `vbaProject.bin`, and 46 with chart/drawing parts. It has 0 with `_xmlsignatures/` package signatures and 0 with the synthetic unknown path family used by `unknown-part-error`. The harness now uses public `fixtures/xlsx/poi/Booleans.xlsx` for `calc-chain-drop` and public `fixtures/xlsx/calamine/date_1904.xlsx` for `docprops-passthrough`. The docProps public case records `passthrough=4`, `regenerate=5`, `drop=0`, `add=0`, `error=0`, keeps the first representative streaming proof, and preserves `docProps/core.xml`, `docProps/app.xml`, and `docProps/custom.xml` as byte-equal passthrough parts in the local probe. `add-sheet-part` adds a second representative streaming proof for the `add` action, and `calc-chain-drop` adds a public streaming proof for the `drop` action once the proof harness carries the recalc dirty signal. `signature-invalidation-drop` and `unknown-part-error` still need generated fixtures or a fixture acquisition policy.
...
Package-action streaming matrix boundary audit: performance can approve only narrow wording today. Current local proof covers 8 package-action cases overall and all five action classes; streaming proof exists for `docprops-passthrough`, `add-sheet-part`, and `calc-chain-drop`. Those streaming cases cover `passthrough`, `regenerate`, `add`, and `drop`, regenerate two worksheet parts across the matrix, and preserve passthrough byte digests. The five non-streaming package-action cases are `regenerate-existing-sheet`, `signature-invalidation-drop`, `macro-passthrough`, `chart-sidecar-accounting`, and `unknown-part-error`.

| Streaming boundary | Current proof evidence | Allowed wording | Forbidden wording |
| --- | --- | --- | --- |
| Representative streaming passthrough/add/drop | Public docProps and calc-chain fixtures plus generated add-sheet proof cover `passthrough`, `regenerate`, `add`, and `drop` in streaming mode. | "Representative streaming proofs cover passthrough/regenerate/add/drop package actions." | "Streaming package-action proof covers every action class." |
| Error actions | Error actions are proven only in non-streaming package-action cases. | "Non-streaming proof covers error package actions." | "Streaming error parity is proven." |
...
| ZIP streaming semantics | Spreadsheet libraries document streaming/write-only tradeoffs and ordering constraints. Ascend has its own streaming writer tests, but this release proof matrix does not cover error, macro, or chart streaming. | "Streaming wording is limited to the tested representative package-action cases." | "Streaming mode is semantically equivalent across all workbook/package features." |

Boundary decision: keep `streaming-matrix-boundary` missing until a performance owner either accepts the representative passthrough/regenerate/add/drop cases as sufficient for release wording or expands the proof matrix. The safe release phrasing is "representative streaming proofs cover passthrough/regenerate/add/drop," not "full streaming parity."
