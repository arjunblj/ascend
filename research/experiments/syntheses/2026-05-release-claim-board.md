...
| **Formula language-service primitives:** Ascend exposes formula IDE building blocks: parse diagnostics, token/reference spans, hover, completions, reference cycling, binding roles, and a rejection-first prepare-rename guard. Allowed wording must say the guard refuses workbook-context and reference targets; it must not imply edit-producing rename. | **Fixture:** SDK rejection matrix already covers LET declarations/uses with shadowing, workbook-context names, table names, table columns, structured item selectors, external workbook refs, 3D refs, spill refs, literals, punctuation, and parse failures.<br>**Benchmark:** assist latency on long formulas and formula corpora is still missing.<br>**Surface:** existing SDK `formulaAssist`, CLI formula assist, API formula-assist, and MCP formula-assist only; no new rename surface.<br>**Validation gate:** parser/span/binding-role tests, cross-surface assist tests, and a future cross-surface `renameTarget` refusal snapshot. Latest local rerun passed SDK formula-edit plus CLI/API/MCP assist tests.<br>**Competitor contrast:** HyperFormula owns formula engine/dependency graph breadth; Ascend should claim workbook-preserving edit intelligence and refusal semantics, not a broader formula IDE.<br>**Honest boundary:** **no edit-producing rename**, no safe workbook-context rename, no table/defined-name rename from formula assist, and no claim that all formula references can be rewritten safely. | Product/DX stewardship only. Proof/spec boundary; not an implementation handoff. |
...
5. **Formula language-service primitives** are allowed only as a rejection-first primitives claim. They still need cross-surface `renameTarget` refusal snapshots and a latency/corpus gate before release copy says more. Do not add rename.
...
- Formula language-service release copy beyond rejection-first primitives until cross-surface refusal snapshots and latency evidence exist.
