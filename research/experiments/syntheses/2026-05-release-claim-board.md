...
Package-action owner acceptance checklist: current proof has 8 cases: 2 public fixtures, 2 generated workbook cases, and 4 generated edge-package cases. Disclosed generated cases are `docprops-passthrough`, `regenerate-existing-sheet`, `add-sheet-part`, `calc-chain-drop`, `signature-invalidation-drop`, and `unknown-part-error`; all five action classes are present with `passthrough=27`, `regenerate=38`, `add=3`, `drop=3`, and `error=1`; source graph evidence and journal package issues appear in every case; `unknown-part-error` is the only post-write audit failure; one representative streaming proof exists. Product must accept or replace generated edge packages. Correctness must approve unsupported-feature boundaries and journal/package issue compatibility. Performance must accept narrow streaming wording or expand the matrix. Release must approve non-provenance wording and compact report publication policy. Until those checkboxes are complete, `package-action-proof` remains `headlineClaimAllowed=false`.
...
Package-action edge fixture policy audit: a local ZIP-entry scan over the 351 checked-in XLSX/XLSM fixtures shows partial public replacement potential, not full replacement. The corpus has 101 workbooks with `xl/calcChain.xml`, 25 with `docProps/custom.xml`, 8 with `customXml/` parts, 2 with `vbaProject.bin`, and 73 with chart/drawing parts. It has 0 with `_xmlsignatures/` package signatures and 0 with the synthetic unknown path family used by `unknown-part-error`. A focused public fixture probe using `fixtures/xlsx/poi/Booleans.xlsx` and a formula edit produced package-action proof counts `passthrough=5`, `regenerate=6`, `drop=1`, `add=0`, `error=0`, with `xl/calcChain.xml` dropped and post-write audits passing. This means `calc-chain-drop` can likely be moved from generated-edge proof to public-fixture proof in a future harness update, but `signature-invalidation-drop` and `unknown-part-error` still need generated fixtures or a fixture acquisition policy.

Boundary decision: keep `edge-fixture-policy` missing. Partial public replacement evidence is useful, but it does not close the product gate because signed and unknown-part edge cases remain unreplaced and because any harness case swap should be owned as a proof-harness change, not silently done in the claim board.
