...
Replacement fixture scan status: `fixtures/benchmarks/safe-open-fixture-scan.ts` scans checked-in public XLSX/XLSM fixtures for `preservedSignature` and `preservedOther` open-plan risk families. The current scan found no replacement candidates in 223 tracked fixtures, so the signed/unknown edge-case blocker remains unresolved rather than merely stale. A constrained external probe found one unknown-part candidate in `node-projects/excelForge/src/test/Book 1.xlsx` with SHA-256 `9c5426fa71ff68cc7e40e19e02b5992daf91da5754ef643d2db2f89bd70bb122`, `preservedOther`, `metadata-only` routing, 50 parts, 37 relationships, and unknown sample part `docMetadata/LabelInfo.xml`. A refresh of the raw package manifest reports license `MIT`, but it is still not a release replacement because it is not checked in here, release owners have not approved binary vendoring/attribution policy, and it does not solve the signed-workbook fixture gap.
